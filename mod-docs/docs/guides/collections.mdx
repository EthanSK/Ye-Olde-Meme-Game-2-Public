...
the `getItemsByMemeId` function has been implemented on the C# side to return a `List<GameObject>`, which is a C# `collection`. Then it automatically gets converted to a `table` in your ModScript, and now you can use Lua's `ipairs` to loop through it.
...
:::warning
There is a caveat to using `createCollection`: after you do this once, MoonSharp will no longer automatically convert a collection of that type to a table. Instead it will just return a reference to the C# collection object. And that happens to all ModScript code everywhere that executes after `createCollection` was called.

<details>
<summary>Why?</summary>

Because `createCollection` needs to (interally) call `registerType` with the provided type, and registering types is global. You could just call `registerType` and pass the C# collection type yourself at the very start, if you want more consistent behaviour from the get-go, but that's up to you.

Or better you could set the execution order of your scripts with `createCollection` to something higher than everything else, so it runs afte and doesn't affect them.
